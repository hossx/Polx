...
要想正确使用币丰港交易平台RESTful API, 建议您仔细阅读我们的「通用规则」，然后再进行具体接口的接入工作。接入过程中，如果遇到任何问题，请联系我们，QQ群：1691767742， 电话：（+86）186-2174-1026。当然，您更可以通过我们的在线客服和我们随时交流。
...
- cursor和limit：在几个API URL参数中，我们会用cursor和limit来指定返回数据的起始位置和数量。返回数据不包括cursor指向的那条数据（如果存在），如果想返回ID是0的数据，cursor的值应该是1(在逆序情况下)。如果没有说明，cursor默认值是“”，limit的默认值是50。每个API对limit可能与上限设置，如果设定的limit值大于这个上限，系统将用该上限作为limit的实际值。
...
- 如果您请求的url不存在或者不正确，我们将返回404错误

- 如果您收到500/服务器错误，请反馈给我们的技术客服，我们将第一时间为您服务。
...
###错误代码表
  ...
  | 2002 | 余额不足 |
  | 2003 | 超过下单最小数量，最小交易额必须大于等值1元人民币(人民币市场)或0.001比特币(比特币市场),提现额度不正确 |
  | 2007 | 不支持的充值提现类型  |
  | 6001 | 该充值提现操作申请已经完成 |
  | 6002 | transfer不存在 |
  | 1010 | 不支持的交易对 |
  ...
  | 9011 | 谷歌密钥不正确 |
  | 9012 | 邮箱验证码不匹配 |
  | 9013 | 输错用户名密码5次，该帐号已经被锁，请120分钟后重试 |
...
####URL参数
- currency：cny或btc,如果提供的currency不是cny或者btc，则返回所有市场的tickers数据。
...
- limit：请参考「术语约定」，默认值50，上限100。
- status：请参考「术语约定」充值提现状态码部分。如果不填，默认值为4。
- type：请参考「术语约定」充值提现部分。如果不填，代表所有类型有返回。
...
####POST参数
- email：必填字段，要注册的邮箱。
- pwdhash：必填字段，要注册的密码。生成方法见json中注释。
- exchangeVersion: 非必须字段, 见注释。
- lang：非必须字段，发送邮件验证码时，使用的邮件模板语言类型, 见注释。
####POST数据JSON格式
####POST数据JSON格式
```
  {
...
####登录失败返回值示例(登录失败重复5次情况下，帐号会被锁定120分钟, canRetryTime表示目前还有几次可以重试)
```
  {
    "canRetryTime": 4,
  }
...
```
  {
    ...
    ], // api密钥对，一个用户可以申请多对api Token
    "emailVerified":true,
    "mobileVerified":true,
    "googleAuthenticatorSecret": SLD823J8DSLD8XM, // 谷歌验证码，如果存在就返回，如果不存在不返回该字段。
...
- cursor：请参考「术语约定」，此处值为交易记录ID，即对应返回数据中的"id"项。交易记录排序按照id大小从大到小排列。
- limit：请参考「术语约定」，默认值100，上限200。
...
- cursor：请参考「术语约定」，此处值为订单ID，即对应返回数据中的"id"项。排序按照id大小从大到小排列。
- limit：请参考「术语约定」，默认值50，上限200。
- order_status：请参考「术语约定」订单状态码部分，默认获取所有状态的订单。
...
- cursor：请参考「术语约定」，此处值为充值记录ID，即对应返回数据中的"id"项。排序按照id大小从大到小排列。
- limit：请参考「术语约定」，默认值50，上限200。
...
批量下单, 一次请求最多下单量为10个。以下所有字段均为必填字段。
